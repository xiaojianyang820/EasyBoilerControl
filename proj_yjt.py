...
class Boiler_YJT(StandardBoiler):
    ...
    def _check_hs_t(self):
        ...
        SQL_Templet = 'SELECT avg(ZG_HS_T) FROM bd_zjyjt_mainstation_2021 WHERE create_time > "%s" AND create_time < "%s" ORDER BY create_time DESC LIMIT 5'
    ...
    def _check_boiler_state(self, boilder_id):
        ...
        cur_time_str = datetime.datetime.strftime(cur_time, '%Y-%m-%d %H:%M:%S')
        start_time_str = datetime.datetime.strftime(start_time, '%Y-%m-%d %H:%M:%S')
        SQL_Templet = 'SELECT avg(GL%d_FH) FROM bd_zjyjt_mainstation_2021 WHERE create_time > "%s" AND create_time < "%s" ORDER BY create_time DESC LIMIT 5'
...
if __name__ == '__main__':
    boiler = Boiler_YJT('boilers/configs/boiler_config_200006.config')
    boiler.main()
