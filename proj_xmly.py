...
class Boiler_XMLY(StandardBoiler):
    ...
    def _dispatch_control_1(self, total_q) -> None:
        ...
        MAX_Q = 8600 * (int(self.config_map['Control_Interval']) / 60)
        MIN_Q = 2100 * (int(self.config_map['Control_Interval']) / 60)
        boiler_id = int(self.config_map['Cur_Boiler_ID'])
        boiler_set_value_max = 70
    ...
    def _check_boiler_state(self, boilder_id):
        ...
        SQL_Templet = 'SELECT avg(GL%d_O_PR) FROM bd_xmly_mainstation_2021 WHERE create_time > "%s" AND create_time < "%s" ORDER BY create_time DESC LIMIT 5'
...
if __name__ == '__main__':
    boiler = Boiler_XMLY('boilers/configs/boiler_config_200001.config')
    boiler.main()
